<x-app-layout>
    <x-container>
        <x-breadcrumb>
            <x-breadcrumb-item :route="route('users.index')">{{ __('Usuarios') }}</x-breadcrumb-item>
        ...
        <x-card>
            <h3>Cargos disponíveis para o usuário <strong>{{ $user->name }}</strong></h3>
            <x-tables.table>
                <x-slot name="thead">
                ...
                <x-slot name="tbody">
                    <form action="{{ route('users.roles.attach', $user->id) }}" method="POST">
                        ...
                        <tr
                        class="border-b transition duration-300 ease-in-out hover:bg-neutral-100 dark:border-neutral-500 dark:hover:bg-neutral-600">
                            <td colspan="3" class="whitespace-nowrap px-6 py-4">{{ __('Nenhum cargo disponível') }}</td>
                        </tr>
                        @endforelse
                        <tr>
                            <td colspan="10">

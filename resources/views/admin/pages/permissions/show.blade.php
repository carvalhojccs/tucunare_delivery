<x-app-layout>  
      ...
      <div class="p-6">
        <div class="flex flex-col">
          ...
          <form>
            ...
            <x-input type="text" class="w-full" name="name" value="{{ $permission->name }}" disabled />
  
            <x-label>Descrição</x-label>
            <x-input type="text" class="w-full" name="description" value="{{ $permission->description }}" disabled />
  
            <div class="flex justify-between">
            
            <div>
              <x-label>Criado em</x-label>
            <x-input type="text" name="created_at" value="{{ $permission->created_at->format('d/m/Y - H:m:s') }}" disabled />
            </div>
            
            <div>
              @if ($permission->created_at != $permission->updated_at)
              <x-label>Última atualização</x-label>
              <x-input type="text" name="updated_at" value="{{ $permission->updated_at->format('d/m/Y - H:m:s') }}" disabled />
            @endif
            </div>
  ...
  >
    ...
    <div class="flex justify-between">
      <div>
        <x-link href="{{ route('permissions.index') }}" class="bg-indigo-600">
        ...
        <x-link href="{{ route('permissions.edit', $permission->id) }}" class="bg-green-500">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
      ...
      <x-form action="{{ route( 'permissions.destroy', $permission->id ) }}" method="DELETE">      
